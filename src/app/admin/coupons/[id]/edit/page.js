...
export default function EditCouponPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (snap.exists()) {
          const c = snap.data();
          setCode(c.code || "");
          setDiscountType(c.type || "percent");
          setDiscountValue(c.value || "");
          setExpiry(
            c.expiry?.toDate
              ? c.expiry.toDate().toISOString().slice(0, 10)
              : ""
          );
          setStatus(c.status || "active");

          // conditions
          setMinAmount(c.conditions?.minAmount || "");
          setMaxAmount(c.conditions?.maxAmount || "");
          setApplyScope(c.conditions?.scope || "all");
          setSelectedCategories(c.conditions?.applicableCategories || []);
          setSelectedProducts(c.conditions?.applicableProducts || []);
          setExcludeProducts(c.conditions?.excludeProducts || []);
          setFirstTimeUser(c.conditions?.firstTimeUser || false);
          setMaxUsage(c.conditions?.maxUsage || "");
          setPerUserLimit(c.conditions?.perUserLimit || "");
        }
      } catch (err) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      window.location.href = "/admin/coupons"; // redirect
    } catch (err) {
      console.error("Error updating coupon:", err);
    }
  };

  // Safe search: by code OR ID
  ...
  if (loading) return <p className="p-6">Loading coupon...</p>;
